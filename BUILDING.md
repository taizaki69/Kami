...
|---|---|
| MihonCompatKit (parsers, VM, repo client, backup reader) | `swift test` on Windows/Swift 6.3.3 — **65/65 tests pass**, including 8 real-APK execution/frontier assertions, 2 receiver-dispatch regressions, and 2 bounded request-model tests; the last published macOS CI baseline passed 63/63 and release `compat-audit` builds |
