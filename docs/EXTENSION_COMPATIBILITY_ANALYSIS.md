...
| DEX execution | **Partial M1/M2 plus two pinned app-facing profiles work**: exact prototype dispatch, resolved reference/catch verification and runtime type checks, receiver-directed virtual selection, maximally specific interface defaults, lexical class/interface super dispatch across parsed DEX graphs, class initialization, stable public source-wrapper routing across measured R8 layouts, pinned constructors/getters, BatCave popular/search/latest/details/chapters/pages, and Kawii Manga popular/search/latest/details/chapters/pages execute through bounded async response delivery and exact compatibility models; the exact locked BatCave and Kawii profiles expose their measured operations through `KamiSource`, while general downloaded-extension compatibility remains open |
| Kotlin/Java class library | **Measured tested subset**: core objects/strings, Kotlin ABI including query trimming/form encoding and bounded string/collection helpers, bounded collections, atomics/reflection, source filters, source-base constructors, bounded OkHttp requests, source-scoped async transport, response/body/Okio values, bounded Jsoup HTML/CSS including direct-child and `:containsData` semantics, generated-serializer JSON encoding/decoding, a measured Java-time subset, and reached `SManga`/`MangasPage`/`SChapter`/`SMangaUpdate`/`Page` models; preferences, additional DOM APIs, and the long tail remain open |
