...
| DEX execution | **Partial M1/M2 slice works**: exact prototype dispatch, class initialization, pinned constructors/getters, and BatCave's pre-request popular path execute; no end-to-end source yet |
| Kotlin/Java class library | **Measured tested subset**: core objects/strings, Kotlin ABI, bounded collections, atomics/reflection, source filters, and source-base constructors; OkHttp and the long tail remain open |
