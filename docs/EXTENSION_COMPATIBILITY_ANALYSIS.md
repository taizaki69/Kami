...
| DEX execution | **Partial M1/M2 slice works**: exact prototype dispatch, resolved reference/catch verification and runtime type checks, receiver-directed virtual selection, maximally specific interface defaults, lexical class/interface super dispatch across parsed DEX graphs, class initialization, pinned constructors/getters, and BatCave popular, paginated text search, latest updates, and core details execute through bounded async response delivery and HTML parsing to exact compatibility models; no complete app-facing source yet |
| Kotlin/Java class library | **Measured tested subset**: core objects/strings, Kotlin ABI including query trimming/form encoding and bounded default collection joining, bounded collections, atomics/reflection, source filters, source-base constructors, bounded OkHttp requests, source-scoped async transport, response/body/Okio values, bounded Jsoup HTML/CSS behavior including reached direct-child relative selectors, and the reached `SManga`/`MangasPage` surface; serialization, preferences, additional DOM APIs, and the long tail remain open |
