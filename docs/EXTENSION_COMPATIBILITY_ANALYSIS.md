...
| DEX execution | **Partial M1/M2 plus one pinned app-facing profile works**: exact prototype dispatch, resolved reference/catch verification and runtime type checks, receiver-directed virtual selection, maximally specific interface defaults, lexical class/interface super dispatch across parsed DEX graphs, class initialization, pinned constructors/getters, and BatCave popular, paginated text search, latest updates, core details, combined chapter updates, and page lists execute through bounded async response delivery and exact compatibility models; the exact locked BatCave profile exposes these through `KamiSource`, while general downloaded-extension compatibility remains open |
