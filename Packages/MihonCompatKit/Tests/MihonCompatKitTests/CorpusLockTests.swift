...
final class CorpusLockTests: XCTestCase {
    ...
    func testCorpusLockHasBoundedSeparatedRolesAndMatchesFetcher() throws {
        ...
        XCTAssertEqual(lock.locked, "2026-08-23")
        ...
        XCTAssertEqual(lock.selection.executionArtifactCount, 5)
        XCTAssertEqual(lock.selection.measurementArtifactCount, 16)
        ...
        XCTAssertEqual(byRole["execution"]?.count, 5)
        XCTAssertEqual(byRole["measurement"]?.count, 16)
        ...
        XCTAssertEqual(Set(byRole["measurement", default: []].map(\.name)), [
            "baozimanhua", "doctruyen3q", "eternalmangas", "foolslidecustomizable",
            "hayalistic", "komga", "komikcast", "mangapandaonl", "mangaplus",
            "mangasoriginesfr", "nhentaixxx", "pixivcomic", "readmanga",
            "sssscanlator", "tuttoanimemanga", "xcomic",
        ])
